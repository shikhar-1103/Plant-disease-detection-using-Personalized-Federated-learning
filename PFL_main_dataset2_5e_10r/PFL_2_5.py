...
def main():
    ...
    LOG_FILE = "final_detailed_5_log.txt"
    CSV_FILE = "final_detailed_5_results.csv"
    ...
    data_dir = r"C:\Users\shikh\Desktop\Thesis\dataset2"
    ...
    def phase_split(client_dataset, n_phases=4):
    ...
    for phase in range(NUM_PHASES):
        ...
        # ---- NEW: Client data 80/20 split for Train/Test ----
        ...
        for client_ds in client_phase_datasets:
            if len(client_ds) == 0:
                client_train_loaders.append(make_loader(client_ds)); client_val_loaders.append(make_loader(client_ds))
                continue
            ...
            val_subset = Subset(client_ds, val_indices)
            
            client_train_loaders.append(make_loader(train_subset))
            client_val_loaders.append(make_loader(val_subset, shuffle=False))
        ...
        old_parent_state = parent_model.state_dict()
        state_c1, state_c2 = client_models[0].state_dict(), client_models[1].state_dict()
        keys_to_aggregate = [k for k in old_parent_state.keys() if 'conv' in k]
        agg_state = {}
        ...
        for model in client_models:
            ...
            for k in keys_to_aggregate: local_state[k] = (local_state[k] + parent_weights[k]) / 2
            model.load_state_dict(local_state)

        # --- Evaluation ---
        pa_train, pp_train, pr_train, pf1_train = compute_metrics(parent_model, parent_loader)
        pa_val, pp_val, pr_val, pf1_val = compute_metrics(parent_model, val_loader)
        
        c1a_train, c1p_train, c1r_train, c1f1_train = compute_metrics(client_models[0], client_train_loaders[0])
        c1a_val, c1p_val, c1r_val, c1f1_val = compute_metrics(client_models[0], client_val_loaders[0])
        
        c2a_train, c2p_train, c2r_train, c2f1_train = compute_metrics(client_models[1], client_train_loaders[1])
        c2a_val, c2p_val, c2r_val, c2f1_val = compute_metrics(client_models[1], client_val_loaders[1])
        
        print(f"Parent | Train Acc: {pa_train:.3f} | Test Acc: {pa_val:.3f}")
        print(f"{CLIENT_NAMES[0]} | Train Acc: {c1a_train:.3f} | Test Acc: {c1a_val:.3f}")
        print(f"{CLIENT_NAMES[1]} | Train Acc: {c2a_train:.3f} | Test Acc: {c2a_val:.3f}")
        
        results_this_phase = { 
            "phase": phase+1, 
            "parent_acc_train": pa_train, "parent_precision_train": pp_train, "parent_recall_train": pr_train, "parent_f1_train": pf1_train,
            "parent_acc_test": pa_val, "parent_precision_test": pp_val, "parent_recall_test": pr_val, "parent_f1_test": pf1_val,
            "client1_acc_train": c1a_train, "client1_precision_train": c1p_train, "client1_recall_train": c1r_train, "client1_f1_train": c1f1_train, 
            "client1_acc_test": c1a_val, "client1_precision_test": c1p_val, "client1_recall_test": c1r_val, "client1_f1_test": c1f1_val,
            "client2_acc_train": c2a_train, "client2_precision_train": c2p_train, "client2_recall_train": c2r_train, "client2_f1_train": c2f1_train,
            "client2_acc_test": c2a_val, "client2_precision_test": c2p_val, "client2_recall_test": c2r_val, "client2_f1_test": c2f1_val,
        ...
        with open(LOG_FILE, "a") as log_file:
            ...
            log_file.write(f"  Accuracy: {pa_val:.4f}, Precision: {pp_val:.4f}, Recall: {pr_val:.4f}, F1-Score: {pf1_val:.4f}\n\n")
            ...
            log_file.write(f"  Accuracy: {c1a_val:.4f}, Precision: {c1p_val:.4f}, Recall: {c1r_val:.4f}, F1-Score: {c1f1_val:.4f}\n\n")
            ...
            log_file.write(f"  Accuracy: {c2a_val:.4f}, Precision: {c2p_val:.4f}, Recall: {c2r_val:.4f}, F1-Score: {c2f1_val:.4f}\n")
            log_file.write("---------------------------------\n")
